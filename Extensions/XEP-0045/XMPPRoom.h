...
typedef enum XMPPRoomAffiliationType
{
  XMPPRoomAffiliationNone    = 0,
  XMPPRoomAffiliationMember  = 1,
  XMPPRoomAffiliationAdmin   = 2,
  XMPPRoomAffiliationOwner   = 3,
  XMPPRoomAffiliationOutcast = 4
} XMPPRoomAffiliationType;

typedef enum _XMPPRoomErrorType {
  XMPPRoomTimeoutErrorType        = 0,
  XMPPRoomAuthenticationErrorType = 1,
  XMPPRoomOtherErrorType          = 2
} XMPPRoomErrorType;

// The error domain that all errors generated by XMPPRoom use
extern NSString* const XMPPRoomErrorDomain;
...
static NSString *const XMPPDiscoInfoNamespace = @"http://jabber.org/protocol/disco#info";
 ...
 */
  ...
  __strong XMPPJID *myJID;
  
	XMPPIDTracker *responseTracker;
  
  __strong NSMutableArray *memberPopulationSet;
...
@property (readonly) XMPPJID * myJID;

@property (readonly) NSString * myNickname; // E.g. robbiehanson

@property (nonatomic, strong, readonly) NSString *roomSubject;

@property (readonly) BOOL isJoined;

@property (nonatomic, assign) BOOL autoRejoin;
...
@optional
- (void)handleRoom:(XMPPRoom *)room subjectMessage:(XMPPMessage *)message;

- (void)handleRoom:(XMPPRoom *)room member:(XMPPJID *)jid 
    joiningMessage:(XMPPMessage *)message;
- (void)handleRoom:(XMPPRoom *)room member:(XMPPJID *)jid
    leavingMessage:(XMPPMessage *)message;

- (void)handleDidCreateRoom:(XMPPRoom *)room;
- (void)handleDidJoinRoom:(XMPPRoom *)room withNickname:(NSString *)nickname;
- (void)handleDidLeaveRoom:(XMPPRoom *)room;

- (void)handleRoom:(XMPPRoom *)room membersList:(NSArray *)items;
...
- (void)xmppRoom:(XMPPRoom *)sender didFetchRoomInfo:(NSXMLElement *)info 
        features:(NSArray *)features;
- (void)xmppRoom:(XMPPRoom *)sender didFailToFetchRoomInfoWithError:(NSError *)error;
...
- (void)xmppRoom:(XMPPRoom *)sender didFailToDestroyRoom:(XMPPIQ *)iqError;
...
- (void)xmppRoom:(XMPPRoom *)sender didSetAffiliationType:(XMPPRoomAffiliationType)type
   withElementID:(NSString *)elementID;
- (void)xmppRoom:(XMPPRoom *)sender didNotSetAffiliationType:(XMPPRoomAffiliationType)type
   withElementID:(NSString *)elementID;

- (void)xmppRoomDidLeavePermanently:(XMPPRoom *)sender;
- (void)xmppRoom:(XMPPRoom *)sender didFailToLeavePermanently:(NSError *)error; 

- (void)xmppRoom:(XMPPRoom *)sender didFailToChangeSubject:(NSString *)newSubject;
- (void)xmppRoom:(XMPPRoom *)sender didChangeSubject:(NSString *)newSubject;
